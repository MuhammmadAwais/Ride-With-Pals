          ...
          <div className="relative z-10">
            ...
            <div className="flex gap-3 flex-wrap">
              <button
                ...
                className="flex-1 bg-[#F5E6D3] text-[#A65E36] py-4 rounded-2xl font-bold hover:bg-[#EBDCC5] transition-all"
              >
                Deposit
              </button>
              <button
                onClick={() => setIsWithdrawModalOpen(true)}
                className="flex-1 bg-black/10 backdrop-blur-sm border border-white/10 py-4 rounded-2xl font-bold hover:bg-black/20 hover:border-white/20 transition-all"
              >
                Withdraw
              </button>
            </div>
            ...
            <div className="flex bg-[#0a0a0a] rounded-xl p-1 border border-white/5">
              {(["30D", "90D", "1Y"] as const).map((r) => (
                <button
                  key={r}
                  onClick={() => setRange(r)}
                  className={`px-4 py-1.5 text-xs rounded-lg ${range === r ? "bg-[#333]" : "text-gray-500"}`}
                >
                  {r}
                </button>
              ))}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {paginatedData.map((item, index) => (
                <tr
                  key={index}
                  className="border-b border-[#1f1f1f] text-sm hover:bg-[#1a1a1a] transition-colors"
                >
                  <td className="py-6 pl-8 font-bold text-gray-200">
                    {item.id}
                  </td>
