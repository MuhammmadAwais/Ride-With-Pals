...
const ManageClubHome = () => {
  ...
  const bgImage = params.get('image') || ''; 
  const clubName = params.get('name') || 'CLUB NAME';
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (openMenuIndex) {
        ...
        if (menuRefs.current[refKey] && !menuRefs.current[refKey]?.contains(event.target as Node)) {
  ...
  const handleCreatePlan = (e: React.FormEvent) => {
    ...
    if (editingPlanId) {
      ...
      setMembershipPlans(membershipPlans.map(p => p.id === editingPlanId ? payload : p));
  ...
  const renderSection = (title: string, sub: string, items: any[]) => (
    <section className="group/section space-y-4 w-full">
      ...
      <div className="w-full bg-[#141414]/80 backdrop-blur-xl rounded-3xl border border-white/[0.06] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] relative">
        ...
        {items.map((item, i) => {
          const isOpen = openMenuIndex?.section === title && openMenuIndex?.index === i;
          return (
            <div 
              key={i} 
                   ...
                   {isOpen && (
                     <div className="absolute right-0 mt-2 w-56 bg-[#181818] rounded-2xl border border-white/10 shadow-2xl z-50 py-2 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                       {title === 'Club Owners' && (
                         <>
                           <button 
                             onClick={() => handleAction('Add as Admin', item.name)}
                           ...
                           <button 
                             onClick={() => handleAction('Remove access of owner', item.name)}
                       ...
                       {title === 'Administrators' && (
                         <button 
                           onClick={() => handleAction('Remove Admin Access', item.name)}
                       ...
                       {title === 'Club Members' && (
                         <>
                           <button 
                             onClick={() => handleAction('Promote to Admin', item.name)}
                             className="w-full text-left px-5 py-3 text-xs font-bold text-gray-300 hover:bg-white/[0.04] hover:text-white transition-colors cursor-pointer"
                           >
                             Promote to Admin
                           </button>
                           <button 
                             onClick={() => handleAction('Promote to Owner', item.name)}
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white p-4 sm:p-8 font-sans w-full">
      {/* Header Banner */}
      <div 
        className="relative h-96 md:h-125 lg:h-150 w-full rounded-3xl mb-8 border border-white/10 shadow-2xl overflow-hidden 
                   flex items-end p-6 sm:p-8 transition-all duration-700 ease-in-out group/banner cursor-pointer"
        style={{
      ...
      >
        <div className="absolute inset-0 bg-black/60 z-0 transition-all duration-500 group-hover/banner:bg-black/40" />

        <div className="absolute top-8 left-8 right-8 flex justify-between z-10">
          <Link to="/dashboard">
            <button 
              onClick={() => navigate(-1)} 
              className="p-3 bg-black/40 hover:bg-white/10 rounded-full border border-white/10 backdrop-blur-md 
                        transition-all duration-300 ease-in-out cursor-pointer shadow-lg shadow-black/40
                        hover:scale-105 active:scale-95 hover:border-white/30 group/btn"
            >
              <svg 
                width="20" 
                height="20" 
          ...
          <button 
            className="px-6 py-3 bg-[#EB712B] hover:bg-[#ff8036] text-white rounded-xl text-xs font-black 
                       tracking-wider uppercase cursor-pointer shadow-lg shadow-[#EB712B]/20 z-20
                       transition-all duration-300 ease-in-out hover:scale-105 active:scale-95 
                       hover:shadow-[0_0_20px_rgba(235,113,43,0.4)] border border-[#EB712B]/30"
          >
            Edit Club
          </button>
        </div>

        {/* Club Info - Pinned to Bottom */}
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full pointer-events-none">
          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-center overflow-hidden shrink-0">
            <img src={bgImage} alt="Club Logo" className="w-full h-full object-cover" />
          </div>
          <div className="flex flex-col justify-center">
            <h1 className="text-3xl sm:text-4xl font-black text-white uppercase tracking-tight break-words">{clubName}</h1>
            <div className="flex items-center gap-3 mt-2">
              <span className="bg-[#EB712B]/30 text-[#EB712B] px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border border-[#EB712B]/50">
                Elite Registry
              </span>
              <span className="text-gray-300 text-xs">Founded 2026</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex gap-1 mb-8 border-b border-white/10 overflow-x-auto scrollbar-hide w-full">
        {['Members', 'Membership Plans', 'Run', 'News', 'Leaderboard'].map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative px-4 py-4 text-xs font-bold uppercase tracking-widest transition-all duration-300 ease-out cursor-pointer whitespace-nowrap ${
            ...
            >
              ...
              {isActive && (
                <span className="absolute bottom-0 left-0 w-full h-[2px] bg-[#EB712B] shadow-[0_0_15px_#EB712B] animate-in fade-in slide-in-from-bottom-2 duration-300" />
              )}
              
              <span className="absolute inset-0 bg-[#EB712B]/5 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-t-lg -z-10" />
            </button>
          );
        })}
      </nav>
          ...
          <div className="w-full bg-[#141414]/80 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl p-8 min-h-[500px] flex flex-col items-center justify-center relative overflow-hidden group transition-all duration-500 hover:border-[#EB712B]/30 hover:shadow-[0_0_40px_rgba(235,113,43,0.15)]">
            ...
            <div className="z-10 flex flex-col items-center max-w-md text-center">
              <div className="w-24 h-24 rounded-full bg-[#1E1E1E] border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)] flex items-center justify-center relative mb-8 transition-transform duration-500 group-hover:scale-105">
                ...
                <CreditCard size={36} className="text-[#635BFF] relative z-10 transition-transform duration-300 group-hover:rotate-6" />
              </div>
              ...
              <button 
                onClick={() => handleAction('Connect to Stripe', clubName)}
                className="px-8 py-4 bg-[#EB712B] text-white rounded-xl text-xs font-black tracking-wider uppercase cursor-pointer shadow-lg shadow-[#EB712B]/20 border border-[#EB712B]/30 relative overflow-hidden transition-all duration-300 ease-out hover:scale-105 active:scale-95 hover:shadow-[0_0_25px_rgba(235,113,43,0.6)] before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-white/20 before:to-transparent before:-translate-x-full hover:before:translate-x-full before:transition-all before:duration-700"
              >
          ...
          <div className="w-full bg-[#181818]/90 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl p-6 md:p-8 animate-in fade-in zoom-in-95 duration-300 transition-all duration-300 hover:border-white/20">
            <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
              <h3 className="text-lg font-black tracking-tight text-white">
                {editingPlanId ? 'Edit Membership Plan' : 'Membership fee plan'}
              </h3>
            ...
            <form onSubmit={handleCreatePlan} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2.5">
                  <label className="text-[10px] font-extrabold text-gray-400 tracking-[0.15em] uppercase">Price</label>
                  <div className="relative">
                    <span className="absolute left-4 top-3.5 text-xs font-black text-gray-500">$</span>
                    <input 
                      type="number" 
                      step="0.01"
                      placeholder="00.00"
                ...
                <div className="flex flex-col gap-2.5">
                  <label className="text-[10px] font-extrabold text-gray-400 tracking-[0.15em] uppercase">Duration</label>
                  <div className="relative group/select">
  <select 
    value={duration}
    onChange={(e) => setDuration(e.target.value)}
    className="w-full bg-[#141414] border border-white/10 rounded-xl px-4 py-3.5 text-xs text-white 
              focus:outline-none focus:border-[#EB712B] focus:ring-1 focus:ring-[#EB712B] 
              appearance-none cursor-pointer font-bold
              transition-all duration-500 ease-in-out
              hover:border-[#EB712B]/50 hover:shadow-[0_0_15px_rgba(235,113,43,0.1)]"
  >
  ...
  <div className="absolute right-4 top-0 bottom-0 flex items-center pointer-events-none transition-transform duration-300 group-hover/select:translate-y-0.5">
    <svg width="10" height="6" viewBox="0 0 10 6" fill="none" stroke="#9CA3AF" className="transition-colors duration-300 group-hover/select:stroke-[#EB712B]">
      <path d="M1 1L5 5L9 1" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2.5">
                  <label className="text-[10px] font-extrabold text-gray-400 tracking-[0.15em] uppercase">Auto Renew</label>
                  <div className="relative">
                    <select 
                      value={autoRenew}
                    ...
                    <div className="absolute right-4 top-0 bottom-0 flex items-center pointer-events-none">
                      <svg width="10" height="6" viewBox="0 0 10 6" fill="none" stroke="#9CA3AF">
                        <path d="M1 1L5 5L9 1" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-2.5">
                  <label className="text-[10px] font-extrabold text-gray-400 tracking-[0.15em] uppercase">Discount (%)</label>
                  <input 
                    type="number" 
                    min="0"
                    max="100"
              ...
              <div className="flex flex-col gap-2.5 pt-2">
                <label className="text-[10px] font-extrabold text-gray-400 tracking-[0.15em] uppercase">Features Inclusion</label>
                    ...
                    {featuresList.map((feat, index) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-bold text-gray-200 tracking-tight">{feat}</span>
                        </div>
                        <button 
                          type="button"
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  <span className="relative z-10">{editingPlanId ? 'Save Changes' : 'Create Membership Plan'}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      <div className="w-full space-y-6 animate-in fade-in slide-in-from-right-5 duration-500">
        ...
        {membershipPlans.length === 0 ? (
          <div className="w-full bg-[#141414]/50 backdrop-blur-xl border border-white/[0.04] rounded-3xl p-12 text-center transition-all duration-300 hover:border-white/10">
            <p className="text-xs font-black text-gray-600 uppercase tracking-[0.15em]">No membership plans added yet</p>
            <p className="text-[10px] text-gray-500 mt-1">Fill out the form on the left to create and preview cards here.</p>
          </div>
        ) : (
          membershipPlans.map((plan) => (
            ...
            >
              ...
              <div className="absolute top-6 right-6 z-40">
                <button 
                  onClick={() => setOpenCardMenuId(openCardMenuId === plan.id ? null : plan.id)}
              ...
              {/* Feature Lists */}
              <div>
                <h4 className="text-[9px] font-extrabold uppercase tracking-widest text-gray-500 mb-3">Included Benefits</h4>
                <ul className="space-y-2">
                  {plan.featuresList.map((feature, idx) => (
                    <li key={idx} className="flex items-center gap-2.5 text-xs text-gray-300 font-medium transition-all duration-300 group-hover:translate-x-1">
              ...
              <div className="grid grid-cols-2 gap-4 bg-[#141414]/50 border border-white/5 rounded-2xl p-4 text-[10px] font-extrabold uppercase tracking-[0.05em] transition-all duration-300 group-hover:bg-[#141414]/80">
                <div>
                  <span className="block text-gray-500 font-bold mb-1">Auto-Renew</span>
                  <span className={`text-xs transition-colors duration-300 ${plan.autoRenew === 'Yes' ? 'text-green-400' : 'text-gray-400'}`}>
                    {plan.autoRenew}
                  </span>
                </div>
                <div>
                  <span className="block text-gray-500 font-bold mb-1">Stripe Status</span>
