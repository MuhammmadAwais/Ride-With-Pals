...
const NewsArticle = ({ item }: { item: typeof newsItems[0] }) => {
  ...
  return (
    <article className="group relative bg-[#0a0a0a] border border-white/[0.05] rounded-3xl p-6 sm:p-8 overflow-hidden transition-all duration-700 ease-out hover:border-[#EB712B]/40 hover:-translate-y-1">
      ...
      <div className="relative flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
        ...
        <div className="w-12 h-12 sm:w-14 sm:h-14 shrink-0 flex items-center justify-center rounded-2xl bg-white/[0.02] border border-white/[0.05] group-hover:bg-[#EB712B] group-hover:scale-105 transition-all duration-500 ease-in-out">
          <Newspaper className="text-[#EB712B] group-hover:text-black transition-colors duration-500" size={24} />
        </div>

        <div className="flex-1 space-y-4 w-full">
          <div className="flex justify-between items-start gap-4">
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white group-hover:text-[#EB712B] transition-colors duration-300 leading-tight">
                {item.title}
              </h2>
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mt-1">{item.date}</p>
            </div>
            <div className="shrink-0 group-hover:translate-x-1 transition-transform duration-300">
              <ArrowUpRight className="text-white/10 group-hover:text-[#EB712B] transition-colors" size={20} />
            </div>
          </div>
