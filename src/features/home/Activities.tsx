...
const girlImages = ["Girlmage1.png", "Girlmage2.png", "Girlmage3.png", "Girlmage4.png", "Girlmage5.png", "GrilImage11.png", "GirlImage11.png", "GirlImage11.png", "Girlmage9.png", "GirlImage10.png"];
const leaderImages = ["Girlmage1.png", "Girlmage2.png", "Girlmage3.png", "Girlmage4.png", "Girlmage5.png", "GrilImage11.png", "GirlImage11.png", "GirlImage11.png", "Girlmage9.png", "GirlImage10.png"];
...
const ActivitiesRegistry = () => {
  ...
  const chartData = {
    ...
    datasets: [
      { label: 'Completed Carpools', data: [3000, 2500, 9500, 4000, 3500, 4500, 4000, 5000, 4500, 5500, 6000, 7000], borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', fill: true, tension: 0.4, pointRadius: 0 },
      { label: 'Ride Bookings', data: [4000, 3500, 3000, 4500, 4000, 3800, 4200, 4800, 4500, 5000, 5500, 6000], borderColor: '#f97316', backgroundColor: 'rgba(249, 115, 22, 0.1)', fill: true, tension: 0.4, pointRadius: 0 }
    ]
  };

  const chartOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false }, ticks: { color: '#71717a' } }, y: { min: 0, max: 10000, grid: { color: '#27272a' }, ticks: { color: '#71717a' } } } };
          ...
          <div className="flex bg-[#111111] p-1.5 rounded-xl border border-zinc-800 w-full md:w-fit">
            {['Active', 'Completed', 'Archived'].map((tab) => (
              <button 
                ...
                className={`flex-1 md:flex-none px-6 py-2.5 text-xs font-bold rounded-lg transition-all duration-300 ${
                  activeTab === tab 
              ...
              <table className="w-full text-left border-collapse min-w-[700px]">
                ...
                <tbody className="divide-y divide-zinc-800/60">
                  {filteredActivities.map((act) => (
                    <tr key={act.id} className="group hover:bg-[#1a1a1a] transition-colors cursor-pointer">
                      <td className="p-6">
                        <div className="flex items-center gap-4">
                          <img src={act.imageUrl} className="w-12 h-12 rounded-2xl object-cover" alt="" />
                          <div>
                            <div className="font-bold text-sm group-hover:text-orange-500 transition-colors">{act.name}</div>
                            <div className="text-xs text-zinc-500">{act.region}</div>
                          </div>
                        </div>
                      </td>
                      <td className="p-6">
                        <img src={act.leaderImageUrl} className="w-10 h-10 rounded-full object-cover border border-zinc-800" alt="" />
                      </td>
                      <td className="p-6">
                        <div className="font-mono text-sm font-bold text-zinc-300">{act.distance}</div>
                        <div className="w-20 h-1.5 bg-zinc-800 rounded-full mt-2 overflow-hidden">
                          <div className="h-full bg-orange-500 rounded-full" style={{ width: `${act.progress}%` }} />
                        </div>
                      </td>
                      <td className="p-6">
                      ...
                      <td className="p-6 text-right">
                        <ChevronRight size={20} className="text-zinc-600 group-hover:text-white transition-transform group-hover:translate-x-1" />
                      </td>
                    </tr>
                  ))}
                </tbody>
