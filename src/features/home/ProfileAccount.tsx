...
const [members, setMembers] = useState([
  ...
  const handleSavePassword = () => {
    ...
    let isValid = true;
    if (!passwordData.current) { newErrors.current = "Current password is required"; isValid = false; }
    if (!passwordData.new) { newErrors.new = "New password is required"; isValid = false; }
    if (passwordData.new !== passwordData.confirm) { newErrors.confirm = "Passwords do not match"; isValid = false; }
    setErrors(newErrors);
    if (isValid) { setIsPasswordModalOpen(false); }
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#121212] text-gray-900 dark:text-white p-4 md:p-12 font-sans transition-colors duration-300">
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="flex items-center gap-2 bg-gray-200 dark:bg-[#111111] px-4 py-2 rounded-xl text-xs font-bold hover:scale-105 transition-all">
        ...
        <div className="bg-white dark:bg-[#111111] p-6 rounded-2xl border border-gray-200 dark:border-white/5 flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-[#1a1a1a] flex items-center justify-center">👤</div>
            <div>
              <h2 className="text-xl font-bold">Rock Climber</h2>
              <p className="text-gray-500 dark:text-[#888] text-sm">Devon Lane</p>
            </div>
          </div>
          <button className="bg-white/5 px-6 py-2 rounded-xl text-sm font-bold hover:bg-[#EB712B] transition-all">Logout</button>
        </div>

        {/* Account Management */}
        <section className="mb-8">
          <h3 className="text-xs text-gray-500 dark:text-[#888] font-bold uppercase mb-4 px-1">Account Management</h3>
          <div className={sectionCardStyle}>
            {[
              { icon: Users, title: "Manage Club", path: "/dashboard/manage-club" },
              { icon: Lock, title: "Change Password", action: () => setIsPasswordModalOpen(true) },
              { icon: ShieldCheck, title: "Permission", action: () => setIsPermissionsModalOpen(true) },
              { icon: Wallet, title: "Wallet", path: "/dashboard/wallet" },
              { icon: CreditCard, title: "Subscription", path: "/dashboard/subscription" },
            ].map((item, idx) => (
              <div key={idx} onClick={item.action || (() => item.path && navigate(item.path))} className={rowItemStyle}>
                <div className="flex items-center gap-4"><item.icon className="text-[#EB712B]" size={20} /><span className="font-medium text-sm">{item.title}</span></div>
        ...
        <section className="border border-red-500/20 bg-red-500/5 p-6 rounded-2xl">
          <h3 className="text-xs text-red-500 font-bold uppercase mb-2 flex items-center gap-2"><AlertTriangle size={14} /> Danger Zone</h3>
          <div className="flex justify-between items-center">
            <p className="text-xs text-red-500/70">Permanently delete your account and all data.</p>
            <button className="border border-red-500/50 text-red-500 px-4 py-2 rounded-lg text-xs font-bold hover:bg-red-500/10">Delete Account</button>
          </div>
        </section>
      </div>
    ...
    <div className="bg-[#1a1a1a] border border-white/10 p-8 rounded-2xl w-full max-w-lg shadow-2xl">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Ensure your account is protected with a strong password.</p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {errors.current && <p className="text-orange-500 text-xs mt-2 font-bold">{errors.current}</p>}
        </div>
        ...
        <div>
          ...
          {errors.new && <p className="text-[#EB712B] text-xs mt-2 font-bold">{errors.new}</p>}
        </div>
        ...
        <div>
          ...
          {errors.confirm && <p className="text-[#EB712B] text-xs mt-2 font-bold">{errors.confirm}</p>}
        </div>
      </div>
      ...
      <div className="flex gap-3 bg-[#111111] p-4 rounded-lg mt-6 border border-white/5 text-gray-400 text-xs">
        ...
        <p>Use at least 8 characters, including a mix of letters, numbers, and symbols. Avoid using common words or names associated with your profile.</p>
      </div>

      {/* Footer Buttons */}
      <div className="flex justify-end gap-4 mt-8">
        <button onClick={() => setIsPasswordModalOpen(false)} className="text-gray-400 font-bold hover:text-white">Cancel</button>
        <button onClick={handleSavePassword} className="px-6 py-2 bg-[#EB712B] rounded-lg font-bold text-white hover:bg-[#d66525] transition-all">Save Changes</button>
      </div>
    </div>
  </div>
)}
    ...
    <div className="bg-[#111111] border border-white/10 p-8 rounded-3xl w-full max-w-md shadow-2xl">
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Manage access levels for your club</p>
        </div>
        <button onClick={() => setIsPermissionsModalOpen(false)} className="text-gray-400 hover:text-white"><X size={20} /></button>
      </div>

      <div className="grid grid-cols-2 gap-4 mb-6">
        {[
          { id: 'admin', label: 'Administrator', desc: 'Full system control and user management.', icon: Shield },
          { id: 'user', label: 'User', desc: 'Standard access to club features.', icon: Users }
        ].map((item) => (
          <button 
  key={item.id} 
  onClick={() => {
...
>
  <div className="flex items-center gap-4">
    <div className={`p-2 rounded-lg ${fullAccess ? 'bg-[#EB712B]' : 'bg-[#1A1A1A]'}`}>
      <Shield size={20} className={fullAccess ? 'text-white' : 'text-[#EB712B]'} />
    </div>
    <div className="text-left">
      <h3 className="text-sm font-bold text-white">Grant Full Club Access</h3>
      <p className="text-[10px] text-gray-500">Unlock all restricted sections</p>
    </div>
  </div>
  <ArrowRight size={20} className="text-gray-500" />
</button>

      <button onClick={() => setIsPermissionsModalOpen(false)} className="w-full bg-[#EB712B] py-3 rounded-xl text-sm font-bold text-white">Save Changes</button>
    </div>
  </div>
)}
    ...
    <div className="bg-[#0f0f0f] border border-white/10 p-8 rounded-3xl w-full max-w-sm shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-[10px] uppercase tracking-widest font-bold mt-1">Granular Control</p>
        </div>
        <button 
      ...
      <div className="space-y-4 mb-8">
        {[
          { key: 'publishRides', label: 'Publish Rides', sub: 'CONTENT', icon: Bike },
          { key: 'publishNews', label: 'Publish News', sub: 'COMMUNICATIONS', icon: Info },
          { key: 'publishDiscount', label: 'Publish Discount', sub: 'MARKETING', icon: CreditCard },
          { key: 'banUsers', label: 'User Moderation', sub: 'SECURITY', icon: Shield },
        ].map((item) => (
          <div key={item.key} className="group bg-[#151515] p-4 rounded-2xl flex items-center justify-between border border-white/5 hover:border-white/10 transition-all">
            <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => setAdminPermissions({...adminPermissions, [item.key]: !adminPermissions[item.key as keyof typeof adminPermissions]})} 
            ...
            >
              <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-300 shadow-sm ${adminPermissions[item.key as keyof typeof adminPermissions] ? 'left-6' : 'left-1'}`} />
            </button>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-3">
        <button onClick={() => setIsAdminSettingsOpen(false)} className="flex-1 py-3 rounded-xl text-sm font-bold text-gray-400 hover:text-white border border-white/5 hover:border-white/10 transition-all">Cancel</button>
<button 
...
{isUserModalOpen && (
  <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[70] p-4 animate-in fade-in duration-300">
    <div className="bg-[#0f0f0f] border border-white/10 p-8 rounded-3xl w-full max-w-sm shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">User Access</h2>
          <p className="text-gray-500 text-[10px] uppercase tracking-widest font-bold mt-1">Standard Controls</p>
        </div>
        <button 
      ...
      <div className="space-y-4 mb-8">
        {[
          ...
          { key: 'publishDiscount', label: 'Publish Discount', icon: CreditCard },
        ].map((item) => (
          <div key={item.key} className="group bg-[#151515] p-4 rounded-2xl flex items-center justify-between border border-white/5 hover:border-white/10 transition-all">
            <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => setTempUserPermissions({...tempUserPermissions, [item.key]: !tempUserPermissions[item.key as keyof typeof tempUserPermissions]})} 
            ...
            >
              <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-300 shadow-sm ${tempUserPermissions[item.key as keyof typeof tempUserPermissions] ? 'left-6' : 'left-1'}`} />
            </button>
          </div>
        ))}
      </div>
...
{isMemberPickerOpen && (
  <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[80] p-4 animate-in fade-in duration-300">
    <div className="bg-[#121212] border border-white/10 p-6 rounded-3xl w-full max-w-sm shadow-[0_0_50px_rgba(0,0,0,0.3)]">
      ...
      {/* Header */}
      <div className="flex justify-between items-center mb-8">
        <button onClick={() => setIsMemberPickerOpen(false)} className="text-gray-400 hover:text-white transition-colors">
          <ChevronLeft size={22} />
        </button>
        <h2 className="text-lg font-bold text-white tracking-wide">Permissions</h2>
        <button onClick={() => setIsMemberPickerOpen(false)} className="text-gray-400 hover:text-white transition-colors">
          <X size={22} />
        </button>
      </div>
      ...
      <div className="space-y-3 mb-6">
        {[
          ...
          >
            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${memberSelectionType === item.id ? 'border-[#EB712B]' : 'border-gray-600'}`}>
              {memberSelectionType === item.id && <div className="w-2.5 h-2.5 bg-[#EB712B] rounded-full" />}
            </div>
            <span className={`text-sm font-bold ${memberSelectionType === item.id ? 'text-white' : 'text-gray-400'}`}>{item.label}</span>
          </button>
        ))}
      </div>
            ...
            {members.map(m => (
              <div key={m.id} className="flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  ...
                  {m.isAdmin && <Shield size={13} className="text-[#EB712B]" />}
                </div>
                <button 
                  onClick={() => setMembers(members.map(mem => mem.id === m.id ? {...mem, selected: !mem.selected} : mem))} 
                ...
                >
                  {m.selected && <Check size={14} className="text-white" />}
                </button>
              </div>
            ))}
          </div>
