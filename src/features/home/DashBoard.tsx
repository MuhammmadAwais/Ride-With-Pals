...
import { 
  ...
  Settings, UserCircle,  Search, X, Trophy, Percent, UserPlus 
} from 'lucide-react';
...
export default function ProfessionalDashboard() {
  ...
  return (
    <div className="flex w-screen h-screen bg-[#111111] text-white font-sans overflow-hidden">
      {/* Sidebar */}
      <aside className={`fixed lg:relative z-20 w-72 h-full bg-[#111111] border-r border-white/10 p-6 flex flex-col transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex justify-between items-center mb-10 px-2 shrink-0">
          <img src="/Images/Logo.png" alt="CORE Logo" className="h-12 w-auto object-contain" />
        ...
        <nav className="flex-1 flex flex-col space-y-2 overflow-y-auto">
          <NavItem onClick={() => setActiveTab('Dashboard')} active={activeTab === 'Dashboard'} label="Dashboard" icon={<LayoutDashboard size={18} />} />
          <NavItem onClick={() => setActiveTab('Activities')} active={activeTab === 'Activities'} label="Activities" icon={<Users size={18} />} />
          <NavItem onClick={() => setActiveTab('Product')} active={activeTab=== 'Produect'} label="Product" icon={<Car size={18} />} />
      ...
      <main className="flex-1 h-full flex flex-col overflow-hidden">
        <header className="h-20 border-b border-white/10 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="w-8 h-8 rounded-full  from-[#EB712B] to-orange-400" />
          </div>
        </header>
...
function KPICard({ title, value, icon }: any) {
  return (
    <div className="bg-[#161616] p-6 rounded-3xl border border-white/5 w-full">
      {icon}
