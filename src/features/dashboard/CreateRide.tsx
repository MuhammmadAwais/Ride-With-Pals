...
import { ArrowLeft, Loader2, CheckCircle2, ChevronDown, Calendar, MapPin, Map, FileCode, FileText } from 'lucide-react';
...
export const CreateRide: React.FC = () => {
  ...
  const editRideId = id || searchParams.get('editId') || searchParams.get('id');
  ...
  const [isLoadingRide, setIsLoadingRide] = useState(Boolean(editRideId));
  ...
  // Fetch Ride Details for Edit Mode
  useEffect(() => {
    if (editRideId) {
      setIsLoadingRide(true);
      backendApi
        .get('/user/ride', { params: { rideId: Number(editRideId) } })
        .then((res) => {
          const rawRide = res.data?.response || res.data?.data || res.data;
          if (!rawRide) {
            toast.error(t`Activity not found.`);
            navigate(ROUTES.ACTIVITIES);
            return;
          }

          let initialTime = rawRide.time || '08:00';
          if (initialTime.length >= 5) {
            initialTime = initialTime.substring(0, 5);
          }

          const leaders = (Array.isArray(rawRide.rideLeaders) ? rawRide.rideLeaders : [])
            .map((l: any) => ({
              userId: Number(l.userId || l.id || l.user?.id),
              name: String(l.name || l.fullName || l.user?.fullName || 'Leader'),
            }))
            .filter((l: any) => Boolean(l.userId) && !isNaN(l.userId));

          const rawDriver = rawRide.supportCarDriver;
          let driverObj = null;
          if (rawDriver && (rawDriver.name || rawDriver.fullName || rawDriver.userId)) {
            driverObj = {
              userId: rawDriver.userId ? Number(rawDriver.userId) : undefined,
              name: String(rawDriver.name || rawDriver.fullName || ''),
            };
          }

          const sportId = Number(rawRide.activityTypeId || 1);
          const subId = Number(rawRide.sportSubTypeId || (sportId === 1 ? 1 : 1));

          dispatch(
            updateStepFields({
              rideName: rawRide.rideName || rawRide.title || rawRide.name || '',
              date: formatDateToISO(rawRide.date),
              time: initialTime,
              activityTypeId: sportId,
              sportSubTypeId: subId,
              isAsphalt: subId === 1,
              isTrail: subId === 2,
              categoryTypeId: Number(rawRide.categoryTypeId || 1),
              meetingPoint: rawRide.meetingPoint || rawRide.location || '',
              endingPoint: rawRide.endingPoint || '',
              gpxFile: rawRide.gpxFile || '',
              distance: Number(rawRide.distance || 0),
              description: rawRide.description || '',
              pace: rawRide.pace || 'Medium',
              elevationGain: Number(rawRide.elevationGain || 0),
              isRecurringActivity: Boolean(rawRide.isRecurringActivity),
              recurringActivities: Array.isArray(rawRide.recurringActivities) ? rawRide.recurringActivities : [],
              expiryDate: formatDateToISO(rawRide.expiryDate || ''),
              isStops: Boolean(rawRide.isStops),
              stops: Array.isArray(rawRide.stops) ? rawRide.stops : [],
              isRecommendedSlots: Boolean(rawRide.isRecommendedSlots),
              recommendedSlots: Array.isArray(rawRide.recommendedSlots) ? rawRide.recommendedSlots : [],
              isPublic: rawRide.isPublic !== undefined ? Boolean(rawRide.isPublic) : true,
              isWomenAndNonBinary: Boolean(rawRide.isWomenAndNonBinary),
              isPaymentRequired: Boolean(rawRide.isPaymentRequired || (rawRide.price && Number(rawRide.price) > 0)),
              price: Number(rawRide.price || 0),
              termsAndConditions: rawRide.termsAndConditions || '',
              rideLeaders: leaders,
              supportCarDriver: driverObj,
              clubId: rawRide.clubId ? Number(rawRide.clubId) : undefined,
              currentStep: 1,
            })
          );
        })
        .catch((err) => {
          console.error('Failed to load ride details for editing', err);
          toast.error(err?.response?.data?.message || err.message || t`Failed to load activity details.`);
          navigate(ROUTES.ACTIVITIES);
        })
        .finally(() => {
  ...
  const handleSubmit = async () => {
    ...
    if (!activeClubId && !formState.clubId) {
      toast.error(t`No active club selected. Please select or create a club first.`);
      return;
    }

    const targetClubId = Number(activeClubId || formState.clubId || 1);
    ...
    try {
      ...
      const formattedDate = formatDateToISO(formState.date);
      const formattedTime = formatTimeTo24h(formState.time);
      ...
      if (!isEditMode) {
        payload.clubIds = [targetClubId];
      } else {
        payload.id = Number(editRideId);
      }

      if (formState.gpxFile) {
        payload.gpxFile = formState.gpxFile;
      }
      ...
      if (formState.endingPoint?.trim()) {
        payload.endingPoint = formState.endingPoint.trim();
      }
      if (formState.termsAndConditions?.trim()) {
        payload.termsAndConditions = formState.termsAndConditions.trim();
      }
      ...
      if (formState.isRecurringActivity && formState.expiryDate) {
        payload.expiryDate = formatDateToISO(formState.expiryDate);
      }
      ...
      if (formState.supportCarDriver) {
        ...
        const driverName = String(driverObj.name || driverObj.fullName || '').trim();
        if (driverName) {
          payload.supportCarDriver = driverId > 0 ? { userId: driverId, name: driverName } : { name: driverName };
        }
      }

      if (isEditMode) {
        // Strictly purge disallowed attributes to prevent backend 400 Bad Request
        ...
        const response = await backendApi.put('/user/ride', payload);
        if (response.status === 200 || response.status === 201) {
          setShowSuccessToast(true);
        } else {
          toast.error(t`Failed to update the ride.`);
        }
      } else {
      ...
      toast.error(err?.response?.data?.message || err.message || (isEditMode ? t`Failed to update the ride.` : t`Failed to host the ride.`));
    } finally {
  ...
  return (
    <div className="p-4 md:p-10 min-h-screen text-text-main bg-main-bg relative overflow-hidden font-sans">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-sm">
          ...
          >
            ...
            <h3 className="text-2xl font-black text-white">
              {isEditMode ? <Trans>Ride Updated!</Trans> : <Trans>Ride Created!</Trans>}
            </h3>
            <p className="text-sm text-text-muted">
              {isEditMode ? (
                <Trans>Your ride <strong>{formState.rideName}</strong> was updated successfully. Redirecting you to activities...</Trans>
              ) : (
      ...
      <div className="max-w-4xl mx-auto space-y-8" ref={dropdownRef}>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-white">
              {isEditMode ? <Trans>Edit Ride</Trans> : <Trans>Add Ride</Trans>}
            </h1>
            <p className="text-text-muted text-xs md:text-sm mt-1">
              {isEditMode ? (
                <Trans>Update activity details and sync changes with your club members.</Trans>
              ) : (
        ...
        {/* Loading Spinner for Initial Edit Fetch */}
        {isLoadingRide ? (
          <div className="bg-surface border border-border rounded-[32px] p-16 shadow-2xl flex flex-col items-center justify-center space-y-4 min-h-[400px]">
            <Loader2 size={40} className="animate-spin text-[#EB712B]" />
            <p className="text-sm text-text-muted font-medium"><Trans>Loading activity details...</Trans></p>
          </div>
        ) : (
          /* Wizard Step Forms */
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2 relative md:col-span-2">
                    ...
                    {openDropdown === 'supportDriver' && (
                      <div className="absolute left-0 w-full bg-main-bg border border-border rounded-2xl shadow-2xl overflow-hidden mt-1 z-30 max-h-60 overflow-y-auto">
                        <div
                  ...
                  {/* Terms & Conditions / Special Rules */}
                  <div className="space-y-3 bg-main-bg p-6 rounded-2xl border border-border md:col-span-2">
                    <div className="flex items-center gap-2.5">
                      <FileText size={18} className="text-[#EB712B]" />
                      <div>
                        <h4 className="text-sm font-bold text-white"><Trans>Terms & Conditions / Rules</Trans></h4>
                        <p className="text-xs text-text-muted"><Trans>Add special rules, waiver details, or safety guidelines for participants.</Trans></p>
                      </div>
                    </div>
                    <textarea
                      value={formState.termsAndConditions || ''}
                      onChange={(e) => dispatch(updateStepFields({ termsAndConditions: e.target.value }))}
                      rows={3}
                      className="w-full bg-surface border border-border rounded-xl p-4 text-sm text-white outline-none focus:border-[#EB712B] transition-colors resize-none placeholder:text-text-muted/40"
                      placeholder={t`e.g. Helmet required, ride at own risk, bring spare tube...`}
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-8">
                  ...
                  <button 
                    ...
                    className="h-16 rounded-2xl bg-[#EB712B] hover:bg-[#ff8243] text-white font-black text-sm uppercase transition-all cursor-pointer border-0 outline-none flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {isSubmitting && <Loader2 size={16} className="animate-spin" />}
                    {isSubmitting ? (
                      isEditMode ? <Trans>Saving Changes...</Trans> : <Trans>Publishing...</Trans>
                    ) : (
                      isEditMode ? <Trans>Save Changes</Trans> : <Trans>Publish Ride</Trans>
                    )}
                  </button>
                </div>
              </div>
